...
pred stepNarrow[pre: SearchState, post: SearchState] {    
    ...
    //////////////////////////////////////////////
    // AFTER CLASS: At hours, we tried this idea! :-)
    // (L+H)/2  -----> L/2 + H/2. There is a risk that we round down *twice* and so 
    // we end up too early in the array (that is, -1 from the actual mid). 
    // Before: let mid = divide[add[pre.low, pre.high], 2] | {
    let mid = add[divide[pre.low, 2], divide[pre.high, 2]] | {
    //////////////////////////////////////////////    
...
pred safeArraySize[arr: IntArray] {
    ...
    // AFTER CLASS: removing this, because we want to see if the edit helps. 
}
...
/*
  ...
  /// AFTER CLASS
  (4/2) + (7/2) = 2 + 3 = 5
  // But in other cases, there may be unreachable area in middle if low and high are both odd??
